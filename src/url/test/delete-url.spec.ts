...
describe('UrlService - deleteUrl', () => {
  ...
  it('deve excluir logicamente a URL se ela existir e pertencer ao usuário', async () => {
    const mockUrl = { id: '1', userId: 'user123', deletedAt: null };
    mockPrisma.url.findUnique.mockResolvedValue(mockUrl);
    mockPrisma.url.update.mockResolvedValue({ ...mockUrl, deletedAt: new Date() });

    const result = await service.deleteUrl('1', 'user123');
  ...
  it('deve lançar exceção se a URL não for encontrada', async () => {
    ...
    await expect(service.deleteUrl('1', 'user123')).rejects.toThrow(NotFoundException);
  });
